...
const fromIcon = L.divIcon({
  className: "",
  html: `<div style="width:16px;height:16px;border-radius:50%;background:#f97316;border:3px solid #fff;box-shadow:0 2px 8px rgba(249,115,22,0.5)"></div>`,
  iconSize: [16, 16], iconAnchor: [8, 8],
});
const toIcon = L.divIcon({
  className: "",
  html: `<div style="width:16px;height:16px;border-radius:50%;background:#fff;border:3px solid #1a6b7a;box-shadow:0 2px 8px rgba(13,79,92,0.3)"></div>`,
  iconSize: [16, 16], iconAnchor: [8, 8],
});
const carIcon = L.divIcon({
  className: "",
  html: `<div style="width:32px;height:32px;border-radius:50%;background:#0d4f5c;border:2px solid #fff;box-shadow:0 3px 12px rgba(13,79,92,0.4);display:flex;align-items:center;justify-content:center;font-size:16px;">🚗</div>`,
...
function MapClickHandler({ onMapClick }: { onMapClick: (p: MapPoint) => void }) {
  useMapEvents({
    click(e) {
...
export default function DispatchMap({
  center, pickup, dropoff, driverLocation, height = "100%", onMapClick,
}: Props) {
    ...
    >
      ...
      {driverLocation && (
        <>
          ...
          <CircleMarker
            center={[driverLocation.lat, driverLocation.lng]} radius={28}
